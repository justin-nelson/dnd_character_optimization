...
g_classLevel = function(build, class_n) { return(sum(build$class == class_n, na.rm=T)) }
...
calculateQualifiedClasses = function(avail_classes, build, preferred_classes=c(), max_level=calculateLevel(build)+1, cooling=0.9){
  ...
  for(class_n in names(cls_av)){
    if(!is.null(cls_av[[class_n]][["prereq_BAB"]])){ if(cls_av[[class_n]][["prereq_BAB"]] > calculateBAB(build)) break } #BAB Check
    if(!is.null(cls_av[[class_n]][["prereq_feats"]])){ if(!hasAllFeats(build, cls_av[[class_n]][["prereq_feats"]])) break } # prereq Feat Check
    if(!is.null(cls_av[[class_n]][["prereq_skills"]])){ 
      if(!all(g_skillMaxes(build, names(cls_av[[class_n]][["prereq_skills"]])) > cls_av[[class_n]][["prereq_skills"]])) break
    }
...
calculateQualifiedFeats = function(build, feat_type){
  ...
  if("CE" %in% c_feats) qual_feats = c(qual_feats, c("Feint", "Deadly_defense"))
  if(calculateBAB(build) >= 6
     & "TWF" %in% c_feats) qual_feats = c(qual_feats, "ITWF")
  if(calculateBAB(build) >= 11
     & "ITWF" %in% c_feats) qual_feats = c(qual_feats, "GTWF")
  ...
  if(calculateLevel(build) >= 21
     & all(c("EpicProwess", "CE") %in% c_feats)) qual_feats = c(qual_feats, "CombatInsight")
  if(calculateLevel(build) >= 21
     & g_classLevel(build, "dervish") >= 6) qual_feats = c(qual_feats, "EpicCharge")
  if(g_classLevel(build, "fighter") >= 4
     & "WF(Kama)" %in% c_feats) qual_feats = c(qual_feats, "WS")
  if("Dodge" %in% c_feats) qual_feats = c(qual_feats, "Mobility")
  if(calculateBAB(build) >= 4
     & "Mobility" %in% c_feats) qual_feats = c(qual_feats, "SpringAttack")
  if(calculateBAB(build) >= 4
     & all(c("Dodge", "Mobility", "CE") %in% c_feats)
     & ("SpringAttack" %in% c_feats || g_classLevel(build, "dervish") >= 3) ) qual_feats = c(qual_feats, "WhirlwindAttack")
  if(g_classLevel(build, "fighter") >= 8
     & "WF(Kama)" %in% c_feats) qual_feats = c(qual_feats, "GWF")
  if(g_classLevel(build, "fighter") >= 12
     & all(c("GWF", "WS") %in% c_feats)) qual_feats = c(qual_feats, "GWS")
