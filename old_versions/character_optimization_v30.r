...
g_feats = function(build){
  ...
  #c_feats = unlist(c_feats)
  
  c_feats = c_feats[!is.na(c_feats)]
...
g_scoreFeats = function(build, feat_list, options=list()){
  ...
  for(ind in 1:length(feat_list)){
    ...
    scores[ind] = evaluateBuild(scoring_build, options, calculateLevel(scoring_build))$score
  }
  return(scores)
}
...
evaluateBuild = function(build, options=list(), level=NULL, levels_to_evaluate=NULL){
  ...
  if(level < 1) level = 1
  if(levels_to_evalute < 1) levels_to_evaluate = 1
  if(levels_to_evaluate > level) levels_to_evalute = level
  ...
  score = sum(min_lv_to_eval:level * damage_per_level, na.rm=T) / sum(min_lv_to_eval:level) * mc_pen_multiplier + prereq_bonus(unlist(build$feats[1:level]))
  ...
  while(length(build$potential_classes) != 0 || start){
    ...
    while(calculateLevel(build) < max_level){
      if( max_level_reached & calculateLevel(build) > 0) {
        ...
        if(evaluateBuild(build, options, calculateLevel(build), level_warp+1)$score <= (cooling * best_build_scores[calculateLevel(build)]) ) { break; }
      }
