...
calculateQualifiedClasses = function(avail_classes, character_classes, character_feats){
  ...
  if(calculateLevel(character_classes) >= 15 & calculateLevel(character_classes) <= 20){ 
    if(calculateLevel(character_classes) + sum(class_table) >= 20){ 
...
prereq_feats = c("Dodge", "CE", "WF(Kukri)", "Mobility")
...
evaluateBuild = function(build, burst_weight=0.5){
  ...
  score = calculateScore(damage_per_level) - multiclassPenalty(build$class) + prereq_bonus(unlist(build$feats))
...
build_character = function(avail_classes, max_level = 30, level_warp = 1, burst_weight=0.5, is.final=T, prefix_build = blank_prefix_build, monoclass_depth=5, report=T){
  ...
  if(max_level > calculateLevel(prefix_build$class) + level_warp){
    build = build_character(avail_classes, max_level=max_level-1, level_warp = level_warp, burst_weight = burst_weight, is.final=F, prefix_build=prefix_build, monoclass_depth = monoclass_depth, report=report)
  } else {
    build = prefix_build
  }
  ...
  if(level_warp+1 <= monoclass_depth){
    for(class_n in names(which(avail_classes > 0))){
      ...
      for(i in (level_warp+1):monoclass_depth){
        ...
        class_build = build_character(avail_builds_class, max_level = max_level, level_warp = level_warp, burst_weight = burst_weight, prefix_build = class_build, monoclass_depth = 0, report=F)
