...
calculateQualifiedClasses = function(avail_classes, character_classes, character_feats){
  ...
  if(all(c("WF(Kukri)", "Feint") %in% character_feats)
     & c("dervish") %in% character_classes
...
calculateQualifiedFeats = function(character_classes, character_feats){
  ...
  if(calculateLevel(character_classes) >= 21
     & "GTWF" %in% character_feats) qual_feats = c(qual_feats, "PTWF")
  ...
  if(calculateLevel(character_classes) >= 21
     & "GWF" %in% character_feats) qual_feats = c(qual_feats, "EWF")
  if(calculateLevel(character_classes) >= 21
     & all(c("GWF", "EWF") %in% character_feats)) qual_feats = c(qual_feats, "EWS")
...
calculateAttacks = function(character_classes, character_feats){
...
calculateDamage = function(character_classes, character_feats){
  damage = 0
  crit_chance = 1
  crit_multiplier = 2
...
build_character = function(avail_classes, cooling_factor_multiplier, high_priority_feats = c()){
  last_shown = c()
  builds_analyzed = 0
  ...
  while(length(potential_character_classes) != 0 || start){
    ...
    while(level < 30){
      # Possible Shortcuts
      if(level >= 15 & level < 20){ if(level + sum(sapply(3-table(character_classes), max, 0)) > 20) break }
      if(level >= 20){ if(any(table(character_classes) < 3)) break }
      
      # Calculate damage
      if(level >= 1){
        damage_rolls = calculateDamage(character_classes, unlist(character_feats))
        attack_rolls = calculateAttacks(character_classes, unlist(character_feats))
        defense = level + 10
        ...
        if(seenL30){
          cooling_factor = (1+(cooling_factor_multiplier/(level)))
          ...
          if(damage_dealt <= (best_damage_per_level[level] / cooling_factor)) break
          if(score <= (best_score_per_level[level] / cooling_factor)) break
        }
      }
      ...
      character_classes[level+1] = potential_character_classes[[level+1]][class_index]
      avail_classes[[character_classes[level+1]]] = avail_classes[[character_classes[level+1]]] - 1
      ...
      if(numberOfNewFeats >= 1){
        for(i in 1:calculateNumberOfNewFeats(character_classes)){
          ...
          if(length(which(high_priority_feats %in% potential_character_feats[[level+1]][[i]]))){
            feat_index = which(potential_character_feats[[level+1]][[i]] %in% high_priority_feats)[1]
          } else {
            feat_index = sample(1:length(potential_character_feats[[level+1]][[i]]), 1)
          }
          character_feats[[level+1]][i] = potential_character_feats[[level+1]][[i]][feat_index]
      ...
      # Level up
      level = level+1
      # print(paste(c(rep("#", level), level), sep="", collapse=""))
      # Calculate damage
      damage_rolls = calculateDamage(character_classes, unlist(character_feats))
      attack_rolls = calculateAttacks(character_classes, unlist(character_feats))
      defense = level + 10
    ...
    if(level == 30){ 
      ...
      if(builds_analyzed %% 1000 == 0) print(builds_analyzed)
      if(builds_analyzed %% 10000 == 0) {
        print(character_classes)
        print(character_classes == last_shown)
        last_shown = character_classes
      }
      ...
      if(score > best_score){
        ...
        best_damage = damage_per_level
        best_damage_per_level = damage_per_level
        print("===== Best Classes so far... =====")
        print(character_classes)
        print(damage_per_level)
        print(paste("best_score: ", best_score, sep=""))
      }
    }
    
    
    # Delevel until an interesting difference can be tried
    while(level > 0){
      ...
      if(length(potential_character_feats[[level]]) != 0){
        if(length(which(high_priority_feats %in% potential_character_feats[[level]][[length(potential_character_feats[[level]])]]))){
          feat_index = which(potential_character_feats[[level]][[length(potential_character_feats[[level]])]] %in% high_priority_feats)[1]
        } else {
          feat_index = sample(1:length(potential_character_feats[[level]][[length(potential_character_feats[[level]])]]), 1)
        }
        
        #feat_index = sample(1:length(potential_character_feats[[level]][[length(potential_character_feats[[level]])]]), 1)
        character_feats[[level]][length(potential_character_feats[[level]])] = potential_character_feats[[level]][[length(potential_character_feats[[level]])]][feat_index]
      ...
      if(length(potential_character_classes[[level]]) != 0){
        ...
        character_classes[level] = potential_character_classes[[level]][class_index]
        avail_classes[[ character_classes[level] ]] = avail_classes[[ character_classes[level] ]] - 1
        ...
        if(numberOfNewFeats >= 1){
          for(i in 1:calculateNumberOfNewFeats(character_classes)){
            ...
            if(length(which(high_priority_feats %in% potential_character_feats[[level]][[i]]))){
              feat_index = which(potential_character_feats[[level]][[i]] %in% high_priority_feats)[1]
            } else {
              feat_index = sample(1:length(potential_character_feats[[level]][[i]]), 1)
            }
           # feat_index = sample(1:length(potential_character_feats[[level]][[i]]), 1)
  ...
  return(list(best_classes=best_classes,
              best_damage_per_level=best_damage_per_level,
...
cooling_factor_multiplier = 1.5 # I suggest keeping this number under 2, anything under ~1 isn't guaranteed to get close to optimal, 0 is greedy
avail_classes = list("monk"=30, "dervish"=10, "invis_blade"=5, "fighter"=30, "weapon_master"=0, "swashbuckler"=5)
#high_priority_feats = c("Dodge", "Mobility", "SpringAttack", "CE", "WhirlwindAttack", "WF(Kama)")
high_priority_feats = c()
swashmaster_char = build_character(avail_classes, cooling_factor_multiplier, high_priority_feats)

avail_classes = list("monk"=3, "dervish"=10, "invis_blade"=5, "fighter"=12, "weapon_master"=0, "swashbuckler"=0)
windmaster_char = build_character(avail_classes, cooling_factor_multiplier, high_priority_feats)
# 
outputCharacter(windmaster_char)
# calculateAttacks(windmaster_char$best_classes, windmaster_char$best_feats)
# calculateDamage(windmaster_char$best_classes, windmaster_char$best_feats)

outputCharacter(swashmaster_char)