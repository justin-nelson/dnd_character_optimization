...
GLOBAL_ord_feat_list = getChoosableFeatList(feats_av)
...
populateFeats = function(avail_classes, build, cooling=0.9, options=list()){
  ...
  ord_feat_list = GLOBAL_ord_feat_list
...
levelFeat = function(build, avail_classes, preferred_classes=list(), max_level=calculateLevel(build)+1, cooling=0.9){
...
levelup = function(build, avail_classes, preferred_classes=list(), max_level=calculateLevel(build)+1, cooling=0.9, options=list()){
...
evaluateBuild = function(build, options=list(), level=NULL, levels_to_evaluate=NULL){
  ...
  score = sum(min_lv_to_eval:level * damage_per_level[min_lv_to_eval:level], na.rm=T) / sum(min_lv_to_eval:level) * mc_pen_multiplier + prereq_bonus(unlist(build$feats[1:level]))
...
build_character = function(avail_classes, max_level = 30, level_warp = 1, options=list(), is.final=T, prefix_build = blank_prefix_build, monoclass_depth=5, cooling=1, report=T, preferred_classes=list()){
  builds_analyzed = 0
  ...
  GLOBAL_ord_feat_list <<- g_orderedBestFeats(build, options)
...
avail_classes = list("Monk"=30, "Dervish"=10, "Invisible Blade"=5, "Fighter"=0, "Swashbuckler"=30)
