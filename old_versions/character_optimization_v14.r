...
calculateClassLevel = function(build, class) { return(sum(build$class == class, na.rm=T)) }

calculateQualifiedClasses = function(avail_classes, build, preferred_classes=c(), max_level=calculateLevel(build)+1){
  qual_class = c()
  character_feats = unlist(build$feats)
  
  if(getAvailability(avail_classes, "fighter")         > calculateClassLevel(build, "fighter")      ){ qual_class = c(qual_class, "fighter")       }
  if(getAvailability(avail_classes, "monk")            > calculateClassLevel(build, "monk")         ){ qual_class = c(qual_class, "monk")          }
  if(getAvailability(avail_classes, "swashbuckler")    > calculateClassLevel(build, "swashbuckler") ){ qual_class = c(qual_class, "swashbuckler")  }
  if(all(c("Dodge", "CE") %in% character_feats) 
     & sum(c("WF(Kukri)", "WF(Kama)") %in% character_feats) 
     & calculateBAB(build$class) >= 5
     & getAvailability(avail_classes, "dervish")       > calculateClassLevel(build, "dervish")      ){ qual_class = c(qual_class, "dervish")       }
  if(all(c("WF(Kukri)", "Feint") %in% character_feats)
     & (c("dervish") %in% build$class || calculateLevel(build) >= 14)
     & getAvailability(avail_classes, "invis_blade")   > calculateClassLevel(build, "invis_blade")  ){ qual_class = c(qual_class, "invis_blade")   }
  if(all(c("WF(Kama)", "Dodge", "Mobility", "WhirlwindAttack", "CE") %in% character_feats)
     & calculateBAB(build$class) >=5
     & getAvailability(avail_classes, "weapon_master") > calculateClassLevel(build, "weapon_master")){ qual_class = c(qual_class, "weapon_master") }
  ...
  qual_class = reduceClassBranchFactor(avail_classes, build, qual_class, max_level, 0.9)
...
reduceClassBranchFactor = function(avail_classes, build, qualified_classes, max_level=calculateLevel(build)+1, cooling=0.9, is.final=T){
  ...
  if(calculateLevel(build) >= max_level) { 
    if(is.final){ return(qualified_classes) } else { return(build) }  
  }
  ...
  for(class_n in qualified_classes){
    branch_build = build
    available_levels = getAvailability(avail_classes, class_n) - calculateClassLevel(branch_build, class_n) 
...
calculateQualifiedFeats = function(build, feat_type){
  ...
  if(calculateBAB(build) >= 1
     & calculateSBlevel(build) == 0) qual_feats = c(qual_feats, c("Weap_Fin"))
  if(calculateBAB(build) >= 1
  ...
  if(calculateFighterLevel(build) >= 4
  ...
  if(calculateBAB(build) >= 4
     ...
     & ("SpringAttack" %in% character_feats || calculateDervishLevel(build) >= 3) ) qual_feats = c(qual_feats, "WhirlwindAttack")
  if(calculateFighterLevel(build) >= 8
     & "WF(Kama)" %in% character_feats) qual_feats = c(qual_feats, "GWF")
  if(calculateFighterLevel(build) >= 12
     & all(c("GWF", "WS") %in% character_feats)) qual_feats = c(qual_feats, "GWS")
  if(calculateFighterLevel(build) >= 12) qual_feats = c(qual_feats, "WeaponMastery")
  if(calculateLevel(build) >= 21
     & "GWF" %in% character_feats
     & calculateFighterLevel(build) >= 12) qual_feats = c(qual_feats, "EWF")
  if(calculateLevel(build) >= 21
     & all(c("GWs", "EWF") %in% character_feats)
     & calculateFighterLevel(build) >= 12) qual_feats = c(qual_feats, "EWS")
...
calculateNumberOfNewFeats = function(build){
  ...
  if(lastCharacterClass(build) == "fighter" & calculateFighterLevel(build) == 1) fighter_feats = fighter_feats+1
  if(lastCharacterClass(build) == "fighter" & calculateFighterLevel(build) %% 2 == 0) fighter_feats = fighter_feats+1
...
calculateAttacks = function(build, burst=T){
  ...
  if(calculateMonkLevel(build) <= 4) attack_bonus = attack_bonus - 1
  if(calculateMonkLevel(build) <= 8) attack_bonus = attack_bonus - 1
  ...
  if("Weap_Fin" %in% character_feats
     || calculateSBlevel(build) >= 1) attack_bonus = attack_bonus + floor((dexterity - 10) / 2)
  ...
  if(calculateWMLevel(build) >= 5) attack_bonus = attack_bonus + 1
  if(burst) {
    attack_bonus = attack_bonus + floor(((calculateDervishLevel(build) - 1) / 2) + 1)
    if("EpicCharge" %in% character_feats) attack_bonus = attack_bonus + 4
  }

  top_attack_bonus = BAB + attack_bonus
  
  if(calculateMonkLevel(build) >= 1)  { main_hand_attacks = c(main_hand_attacks, top_attack_bonus); number_of_attacks = number_of_attacks - 1; }
  if(calculateMonkLevel(build) >= 11) { main_hand_attacks = c(main_hand_attacks, top_attack_bonus); number_of_attacks = number_of_attacks - 1; }
  
  if(calculateMonkLevel(build) >= 1 & number_off_hand_attacks > 0)   { off_hand_attacks = c(off_hand_attacks, top_attack_bonus); number_off_hand_attacks = number_off_hand_attacks - 1; }
  if(calculateMonkLevel(build) >= 11 & number_off_hand_attacks > 0)  { off_hand_attacks = c(off_hand_attacks, top_attack_bonus); number_off_hand_attacks = number_off_hand_attacks - 1; }
...
calculateDamage = function(build, burst=T){
  ...
  if(calculateSBlevel(build) >= 5) damage = damage + floor((starting_intelligence - 10) / 2)
  if(calculateDervishLevel(build) == 10) damage = damage + 14/2
  if(calculateWMLevel(build) >= 5) crit_multiplier = crit_multiplier + 1
  if(calculateWMLevel(build) >= 7) crit_chance = crit_chance + 2
  if("IC" %in% character_feats) crit_chance = crit_chance * 2
  
  if(burst){
    damage = damage + ((calculateDervishLevel(build) - 1) / 2) + 1
  ...
  damage = damage + 6 * floor((calculateIBLevel(build) + 1) / 2)
...
levelClass = function(build, avail_classes, preferred_classes=c(), max_level=calculateLevel(build)+1){
  cur_lv = calculateLevel(build)
  if(length(build$potential_classes) < cur_lv+1){
    build$potential_classes[[cur_lv+1]] = calculateQualifiedClasses(avail_classes, build, preferred_classes, max_level)
  }
...
levelup = function(build, avail_classes, preferred_classes=c(), max_level=calculateLevel(build)+1){
  build = levelClass(build, avail_classes, preferred_classes, max_level)
...
build_character = function(avail_classes, max_level = 30, level_warp = 1, burst_weight=0.5, is.final=T, prefix_build = blank_prefix_build, monoclass_depth=5, cooling=1, report=T, preferred_classes=c()){
  ...
  while(length(build$potential_classes) != 0 || start){
    ...
    while(calculateLevel(build) < max_level){
      ...
      build = levelup(build, avail_classes, preferred_classes, max_level)
    }
  ...
  if(level_warp+1 <= monoclass_depth){
    for(class_n in names(which(avail_classes > 0))){
      ...
      if(calculateClassLevel(best_build, class_n) > getAvailability(avail_classes, class_n)) { break }
      ...
      for(i in (level_warp+1):monoclass_depth){
        ...
        if(any(calculateQualifiedClasses(avail_classes, class_build) == "any")){ break }
        if(calculateClassLevel(class_build, class_n) + i > getAvailability(avail_classes, class_n))  { break }
