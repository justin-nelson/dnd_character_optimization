...
populateFeats = function(avail_classes, build, cooling=0.9, options=list()){
  if(length(feats) > calculateLevel(build)) return(build)
  
  for(i in length(feats):calculateLevel(build)){
    ...
    if(number_of_feats$total > 0){ # There should be a feat at this level
      if(length(build$feats) < i){ # But if there isn't, we need to add some ... optimally
        ...
        while(length(unlist(level_build$potential_feats, F, F)) > 0){
