...
build_character = function(avail_classes, max_level = 30, level_warp = 1, burst_weight=0.5, is.final=T, prefix_build = blank_prefix_build){
  ...
  if(max_level >= calculateLevel(prefix_build$class) + level_warp + 1){
    build = build_character(avail_classes, max_level=max_level-1, level_warp = level_warp, is.final=F, prefix_build=prefix_build)
  } else {
    build = prefix_build
  }
...
avail_classes = list("monk"=18, "dervish"=0, "invis_blade"=0, "fighter"=0, "weapon_master"=0, "swashbuckler"=0)
windmaster_char = build_character(avail_classes, max_level = max_level, level_warp = level_warp, burst_weight = burst_weight)

windmaster_25 = createPrefixBuild(windmaster_char, 5, F)
windmaster_30 = build_character(avail_classes, max_level = max_level, level_warp = level_warp, burst_weight = burst_weight, prefix_build = windmaster_25)