...
build_character = function(avail_classes, cooling_factor_multiplier, high_priority_feats = c(), burst_weight=0.5, level_slack = 8){
  last_shown = c()
  builds_analyzed = 0
  ...
  while(length(potential_character_classes) != 0 || start){
    ...
    while(level < 30){
      ...
      if(level >= 1){
        ...
        if(seenL30){
          cooling_factor = (1+((cooling_factor_multiplier * cooling_factor_multiplier)/(level * level)))
          ...
          if(level >= local_min_level+level_slack){
            if(damage_dealt <= (best_damage_per_level_perm[level] / cooling_factor)) break
            if(score <= (best_score_per_level_perm[level] / cooling_factor)) break
          }
        }
      }
      ...
      class_index = sample(1:length(potential_character_classes[[level+1]]), 1)
        # If we are about to run into the 3b20 rule, I need to enforce certain classes. Otherwise just break.
      if(level >= 15 & level < 20){ 
        if(level + sum(sapply(3-table(character_classes), max, 0)) == 20){
          classes_before_3b20 = which(potential_character_classes[[level+1]] %in% which(table(character_classes) < 3))
          if(length(classes_before_3b20) >= 0){
            class_index = classes_before_3b20[1]
          } else {
            potential_character_classes[[level+1]] = NULL
            break;
          }
    ...
    time_since_last_best = time_since_last_best + 1
  ...
  return(list(best_classes=best_classes,
              ...
              noted_builds = noted_builds,
              burst_weight = burst_weight)
  )
  
}

avail_classes = list("monk"=30, "dervish"=10, "invis_blade"=5, "fighter"=30, "weapon_master"=7, "swashbuckler"=5)
cooling_factor_multiplier = 1
burst_weight = 0.5
windmaster_char = build_character(avail_classes, cooling_factor_multiplier = cooling_factor_multiplier, burst_weight=burst_weight)
